...
[![Pipeline Status](https://whitgit.whitworth.edu/2026/spring/CS-472-2/groupprojects/whitworth-room-inventory/whitworth-room-inventory-backend/badges/release/pipeline.svg)](https://whitgit.whitworth.edu/2026/spring/CS-472-2/groupprojects/whitworth-room-inventory/whitworth-room-inventory-backend/pipelines)
...
***Instructions for setting up back end connection:***

**#1.** Ensure that the requirments in requirements.txt are installed properly
pip install -r requirements.txt

**#2.** Create a MySql database connection with a blank schema inside, and make sure the name of the schema is properly noted.

**#3.** Fill in the .env file with the appropriate and coinciding information that is required. For example.

DB_USERNAME= "Data base username"

DB_PASSWORD= "Data base password"

DB_HOST="Local host for now"

DB_PORT=3306

DB_DATABASE="Data base schema name"

**#4.** Run Data.py and while running, plug in http://localhost:5000/resetdb in your local broser to initialize or reset the current data saved in the database, then see if the test checks in testcalls.txt are properly working by also running them in your local browser.

**closing note** If everthing has been done in the proper order, then the front end side of the programe should be properly working and connecting to the backend. Be sure to have the backend running while operating the frontend to properly save information. Some features of the frontend may not work properly or lock a user out if the back end is not properly established.
